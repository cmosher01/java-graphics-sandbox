...
*/
package nu.mine.mosher.zoom.xylophone;
...
public class Xylophone extends Application {
    ...
    public static class Cam extends Group {
...
//                    .translateZ(-0.5*size)
//                    .build()
                    createFace(size, size, color.deriveColor(0.0, 1.0, (1 - 0.5 * shade), 1.0), -0.5 * size, -0.5 * size, 0.5 * size, null, 0),

                    // Bottom face
                    createFace(size, size, color.deriveColor(0.0, 1.0, (1 - 0.4 * shade), 1.0), -0.5 * size, 0, 0, Rotate.X_AXIS, 90),

                    // Right face
                    createFace(size, size, color.deriveColor(0.0, 1.0, (1 - 0.3 * shade), 1.0), -1 * size, -0.5 * size, 0, Rotate.Y_AXIS, 90),

                    // Left face
                    createFace(size, size, color.deriveColor(0.0, 1.0, (1 - 0.2 * shade), 1.0), 0, -0.5 * size, 0, Rotate.Y_AXIS, 90),

                    // Top face
                    createFace(size, size, color.deriveColor(0.0, 1.0, (1 - 0.1 * shade), 1.0), -0.5 * size, -1 * size, 0, Rotate.X_AXIS, 90),

                    // Front face (labeled "top face" in your original code)
                    createFace(size, size, color, -0.5 * size, -0.5 * size, -0.5 * size, null, 0)

            );
        }
    }

    private Rectangle createFace(double width, double height, Color fill, double tx, double ty, double tz, Point3D rotationAxis, double angle) {
        Rectangle rect = new Rectangle(width, height);
        rect.setFill(fill);
        rect.setTranslateX(tx);
        rect.setTranslateY(ty);
        rect.setTranslateZ(tz);

        if (rotationAxis != null && angle != 0) {
            rect.setRotationAxis(rotationAxis);
